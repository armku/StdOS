...
{	
	switch(speed)
	{
		case _10_SPS  :  AD_SPEED=_10_SPS;
			break;
		case _80_SPS  :  AD_SPEED=_80_SPS;
			break;
 
		default: AD_SPEED=_10_SPS;
	}	
}
//配置放大倍数
void ADS123X_GAIN_CONFIG(unsigned char gain)
{	
	switch(gain)
	{
		case ADC_GX_1  :AD_GAIN0 = 0;AD_GAIN1 = 0;	//GAIN = 1
			break;
		case ADC_GX_2  :AD_GAIN0 = 1;AD_GAIN1 = 0;	//GAIN = 2
			break;
		case ADC_GX_64 :AD_GAIN0 = 0;AD_GAIN1 = 1;	//GAIN = 64
			break;
		case ADC_GX_128:AD_GAIN0 = 1;AD_GAIN1 = 1;	//GAIN = 128
			break;	
		
		default: AD_GAIN0 = 0;AD_GAIN1 = 0;	//GAIN = 1
	}	
}
void ADS123x_Init(void)
{
	   unsigned  char i=0;
	 	AD_PDWN  = 0;	DelayUs(20);//使其复位 		 
		ADS123X_SPEED_CONFIG(_80_SPS);
		ADS123X_GAIN_CONFIG(ADC_GX_128);
		AD_SCLK  = 1; DelayUs(20);
		AD_SCLK  = 0;	DelayUs(20);							//拉高RDY
		AD_PDWN  = 1;	DelayUs(20);							//拉高RST
		DelayMs(10);								//等待10MS
		AD_Dout=1;
		DelayMs(10);

        while(AD_Dout);
        for(i=0;i<26;i++) //发送26个脉冲，校准                                                                                               //?26?????ADS1232
        {
                 AD_SCLK=1;
                	_nop_();
                AD_SCLK=0;               
