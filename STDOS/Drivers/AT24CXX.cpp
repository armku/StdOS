...
int AT24CXX::bufwr(uint16_t addr, byte *buf, uint32_t size, uint16_t bufpos, byte wr) //读写集中操作1写 0读
{
...
int AT24CXX::writePage(byte *buf, uint16_t bufpos, uint16_t addr, uint32_t size) //页内写
{
	Buffer bs(buf + bufpos, size);
...
int AT24CXX::readPage(byte *buf, uint16_t bufpos, uint16_t addr, uint32_t size) //页内读
{
	Buffer bs(buf + bufpos, size);
