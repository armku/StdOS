...
//返回值:0,成功;其他:错误代码.
//月份数据表											 
byte const table_week[12] = 
{
    0, 3, 3, 6, 1, 4, 6, 2, 5, 0, 3, 5
}; //月修正数据表	  
//平年的月份日期表
const byte mon_table[12] = 
{
...
DateTime &DateTime::operator = (int seconds)
{
    ...
    //超过一天了
    {
        ...
        //超过了一个月
        {
            if (Is_Leap_Year(this->Year) && temp1 == 1)
            //当年是不是闰年/2月份
            {
                if (temp >= 29)
                    temp -= 29;
