#include "AT24CXX_ARMKU.h"
#include "Sys.h"

#define macI2C_WR	0		/* 写控制bit */
#define macI2C_RD	1		/* 读控制bit */

bool AT24CXX_ARMKU::Write(uint addr, void * buf, int len, int bufpos)
{
	uint curAddr;
...
bool AT24CXX_ARMKU::Read(uint addr, void * buf, int len, int bufpos)
{
	uint curAddr;
	uint pageStart; //页内起始地址
	uint bytesLeave; //还剩多少字节读取
...
//写延时时间
AT24CXX_ARMKU::AT24CXX_ARMKU(EW24XXType devtype, byte devaddr, uint wnms)
{
...
void AT24CXX_ARMKU::SetPin(Pin pinscl, Pin pinsda, Pin pinwriteprotect)
{
...
byte AT24CXX_ARMKU::Read(ushort address)
{
	byte ret = 0;
...
bool AT24CXX_ARMKU::Write(ushort address, byte da)
{
	uint m;
...
//页内读，最多一页
int AT24CXX_ARMKU::PageRead(ushort addr, void * buf, int len)
{
	if (len > this->Block)
		return 1;
...
//页内写，最多一页
int AT24CXX_ARMKU::PageWrite(ushort addr, void * buf, int len)
{
	if (len > this->Block)
		return 1;
	uint m;
...
ushort AT24CXX_ARMKU::jsPageSize(uint type) //计算存储页大小
{
	ushort ret = 8;
	switch (type)
	{
...
byte AT24CXX_ARMKU::checkDevice()
{
	byte ucAck;
