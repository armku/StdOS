...
extern "C" {
	...
	void SetBufFloat(byte buf[], float da, ushort pos = 0, byte type = 0);//设置浮点数 0:小端 1：大端
	float GetBufFloat(byte buf[], ushort pos = 0, byte type = 0);//设置浮点数 0:小端 1：大端
	ushort GetBufUshort(byte buf[], ushort pos, byte type = 0);
	void SetBufUshort(byte buf[], ushort pos, ushort da, byte type = 0); //从电阻获得温度
