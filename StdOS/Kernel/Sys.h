...
public:
    ...
    void Sleep112233(int ms) const; // 毫秒级延迟
...
public:
	...
	uint32_t AddTask(Action func, void* param, int dueTime = 0, int period = 0, cstring name = NULL) const;
	template<typename T>
	uint32_t AddTask(void(T::*func)(), T* target, int dueTime = 0, int period = 0, cstring name = NULL)
	{
