...
StdLink::StdLink(USART &uart) :com(uart)
{
	com.ClearRxBuf();
	com.ClearTxBuf();
}

bool StdLink::CheckFrame()
{
	return this->CheckFrame(rxFrame);
}
bool StdLink::CheckFrame(DataFrame &df)
{
#if 0
...
bool StdLink::Get(uint8_t & da)
{
	return true;
}
bool StdLink::Gets(uint8_t * pData, uint16_t num)
{
	return true;
}
