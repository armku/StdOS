...
#define _LCD_DR_H
    ...
    class CLcd_DR: public CLcd
    {
        public:
            ...
            void DisplayLattice(uint16_t x, uint16_t y, uint8_t *dp, Font font); //显示点阵图形
            ...
            void DispValu(uint16_t x, uint16_t y, uint16_t val, int wei =  - 1, uint8_t reversebit = 0, Font font = FONT_8X16); //显示数值 固定位置反显

        private:
            uint16_t SearchhzIndex(uint8_t lo, uint8_t hi, Font font); //查找汉字编码位置
    };
#endif
