...
class CW25Qxxx
{
    ...
    public:
        ...
        void BufferWrite(byte *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite);
        void BufferRead(byte *pBuffer, uint32_t ReadAddr, uint16_t NumByteToRead);
    private:
        void WAKEUP(void);
        void WaitForWriteEnd(void);
        void WriteEnable(void);               
        void StartReadSequence(uint32_t ReadAddr);
        void PageWrite(byte *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite);
        void BulkErase(void);
	private:
		BasePort *pincs;
		BasePort *pinclk;
